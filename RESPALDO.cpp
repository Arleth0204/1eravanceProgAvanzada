...
LRESULT CALLBACK MANEJODECITASCALLBACK(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND: {
        ...
        case IDNAME: {
            WCHAR textow[225];
            GetDlgItemText(hWnd, IDC_nameusuario, textow, 255);
            //MessageBox(hWnd, textow, L"El nombre del cliente:", 0);//
            SendMessage(GetDlgItem(hWnd, IDC_LIST3), LB_ADDSTRING, 0, (LPARAM)(textow));
        }break;
    ...
    case WM_COMMAND:
        {
            ...
            case IDC_Guardar: {
               WCHAR textow[225];
               GetDlgItemText(hWnd, IDC_EDITusuario, textow, 255);
               MessageBox(hWnd, textow, L"Usuario:", 0);
